...
test.describe('Users Dashboard', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page
      .getByRole('textbox', { name: 'Password * eye' })
      .fill('123456789');
  ...
  test('displays users list', async ({ page }) => {
    ...
    await expect(
      page.getByRole('button', { name: 'Nom', exact: true })
    ).toBeVisible();
  ...
  test('can sort by Prénom column', async ({ page }) => {
    ...
    // Verify the table still displays users
    await expect(
      page.getByRole('cell', { name: 'bob.johnson@example.com' })
    ).toBeVisible();
  });
  ...
  test('can create a new user', async ({ page }) => {
    ...
    await page
      .getByRole('textbox', { name: 'Email *' })
      .fill('alice.williams@example.com');
  ...
  test('can edit an existing user', async ({ page }) => {
    // Click on a user row
    await page
      .getByRole('row', { name: /Jane Smith jane.smith@example/ })
      .click();
